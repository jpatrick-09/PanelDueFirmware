...
//Function to Highlight Set button on fan, temp, and extrusion popup menus
PopupWindow *CreateIntPopupBar(const ColourScheme& colours, PixelNumber width, unsigned int numEntries, const char* const text[], const int * null params, Event ev, Event zeroEv, const uint highlightButton)
{
	PopupWindow *pf = new PopupWindow(popupBarHeight, width, colours.popupBackColour, colours.popupBorderColour);
	DisplayField::SetDefaultColours(colours.popupButtonTextColour, colours.popupButtonBackColour);
	PixelNumber step = (width - 2 * popupSideMargin + popupFieldSpacing)/numEntries;
	for (unsigned int i = 0; i < numEntries; ++i)
	{
		...
		if (i==highlightButton){
			DisplayField::SetDefaultColours(colours.popupButtonTextColour, colours.buttonPressedBackColour);
		} else {
			DisplayField::SetDefaultColours(colours.popupButtonTextColour, colours.popupButtonBackColour);
		}
		pf->AddField(new TextButton(popupSideMargin, popupSideMargin + i * step, step - popupFieldSpacing, text[i], (params[i] == 0) ? zeroEv : ev, iParam));
...
void CreateIntegerAdjustPopup(const ColourScheme& colours)
{
	...
	static const char* const tempPopupText[] = {"-10", "-5", strings->set, "+5", "+10"};
	static const int tempPopupParams[] = { -10, -5, 0, 5, 10 };
	setTempPopup = CreateIntPopupBar(colours, tempPopupBarWidth, 5, tempPopupText, tempPopupParams, evAdjustInt, evSetInt,2);
}
...
void CreateMovePopup(const ColourScheme& colours)
{
	...
	Event e = evMoveX;
	PixelNumber column = movePopupWidth/8;
	...
	for (size_t i = 0; i < MaxAxes; ++i)
	{
		...
		DisplayField::SetDefaultColours(colours.popupTextColour, colours.popupBackColour);
		FloatField *f = new FloatField((row2 + rowHeight/3), column, xyFieldWidth, TextAlignment::Centre, (i == 2) ? 2 : 1, axisNames[i]);
...
{
	static const char * array const babystepStrings[2] = { LESS_ARROW " 0.05", MORE_ARROW " 0.05" };
...
void CreateControlTabFields(const ColourScheme& colours)
{
	...
	DisplayField::SetDefaultColours(colours.buttonTextColour, colours.buttonTextBackColour);
	filesButton = AddTextButton(row8p7, 0, 4, strings->files, evListFiles, nullptr);
...
namespace UI
{
	...
	void ShowFilesButton()
	{
		...
		mgr.Show(bedCompButton, true);
		mgr.Show(homeAllButton, true);
		for (auto singleButton:homeButtons){
			mgr.Show(singleButton, true);
		}
	}
	...
	void ShowPauseButton()
	{
		...
		mgr.Show(bedCompButton, false);
		mgr.Show(homeAllButton, false);
		for (auto singleButton:homeButtons){
			mgr.Show(singleButton, false);
		}
	}
